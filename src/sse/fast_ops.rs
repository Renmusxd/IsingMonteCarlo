...
pub struct FastOpsTemplate<O: Op, ALLOC: FastOpAllocator = DefaultFastOpAllocator> {
    ...
    pub(crate) n: usize,
    pub(crate) loc_ends: Option<(OpIndex, OpIndex)>,
...
impl<O: Op + Clone, ALLOC: FastOpAllocator> FastOpsTemplate<O, ALLOC> {
        ...
        I: IntoIterator<Item = (usize, O)>,
    {
        ...
        let last_loc: Option<OpIndex> = None;
        let mut last_prels: Vec<Option<PRel>> = self.get_instance();
...
{
    pub(crate) op: O,
    pub(crate) previous_loc: Option<OpIndex>,
    pub(crate) next_loc: Option<OpIndex>,
...
pub struct FastOpMutateArgs {
    /// Last index seen
    last_loc: Option<OpIndex>,
    /// Last p seen per var
    last_prels: Vec<Option<PRel>>,
...
impl FastOpMutateArgs {
    ...
    where
        Fact: Factory<Vec<usize>> + Factory<Vec<Option<PRel>>>,
    {
        let mut last_prels: Vec<Option<PRel>> = fact.get_instance();
...
impl<O: Op + Clone, ALLOC: FastOpAllocator> DiagonalSubsection for FastOpsTemplate<O, ALLOC> {
        ...
        F: Fn(&Self, Option<&Self::Op>, T) -> (Option<Option<Self::Op>>, T),
    {
        ...
        //         );
        //         node_ref.previous_loc = old_op_node.previous_p;
        //         node_ref.next_loc = old_op_node.next_p;
        ...
        F: Fn(&Self, &Self::Op, OpIndex, T) -> (Option<Option<Self::Op>>, T),
    {
        ...
        //             debug_assert!(next_p.map(|next| next.loc >= pstart).unwrap_or(true));
        //
        //             // Push the p just inside the pstart-pend range.
        //             if let Some(PRel { loc: next_p, .. }) = next_p {
        //                 p_heap.push(Reverse(next_p));
        //             }
        //         } else if let Some((start, _)) = self.var_ends[var] {
        //             debug_assert!(start.loc >= pstart);
        //             p_heap.push(Reverse(start.loc));
        //         }
        //     });
        //
    ...
    {
        let psel = p;
        let iter_and_set = |mut loc_check: OpIndex,
                            var: usize,
                            mut relv: usize,
                            subvar: usize,
                            args: &mut Self::Args| {
        ...
        let set_using = |prel: PRel, subvar: usize, args: &mut Self::Args| {
        ...
        hint.into_iter()
            ...
            .enumerate()
            .for_each(
                |(subvar, (loc_hint, var)): (usize, (Option<OpIndex>, usize))| {
                    ...
                    let can_use_start_iter = var_start
                        ...
                        .unwrap_or(false);
                    let use_exact: Option<PRel> = match (loc_hint, phint, var_start) {
        ...
        It: IntoIterator<Item = Option<OpIndex>>,
    {
        ...
        let set_using = |loc_hint: OpIndex, relv: usize, subvar: usize, substate: &mut [bool]| {
        ...
        hint.into_iter()
            ...
            .for_each(|(subvar, it)| {
                ...
                let (loc_hint, var): (Option<OpIndex>, usize) = it;
                debug_assert!(
                    loc_hint
                        .map(|loc_hint| self
                ...
                let var_start: Option<PRel> = self.var_ends[var].map(|(prel, _)| prel);
                let loc_hint = loc_hint.and_then(|mut loc_hint: OpIndex| {
...
impl<O: Op + Clone, ALLOC: FastOpAllocator> Factory<Vec<OpIndex>> for FastOpsTemplate<O, ALLOC> {
...
impl<O: Op + Clone, ALLOC: FastOpAllocator> Factory<Vec<Option<OpIndex>>>
    for FastOpsTemplate<O, ALLOC>
{
...
impl<O: Op + Clone, ALLOC: FastOpAllocator> Factory<Vec<Option<PRel>>>
    for FastOpsTemplate<O, ALLOC>
{
...
impl<O: Op + Clone, ALLOC: FastOpAllocator> RvbUpdater for FastOpsTemplate<O, ALLOC> {
    fn constant_ops_on_var(&self, var: usize, ps: &mut Vec<OpIndex>) {
    ...
    fn spin_flips_on_var(&self, var: usize, ps: &mut Vec<OpIndex>) {
        let mut p_and_rel = self.get_first_prel_for_var(var);
        while let Some(PRel {
            loc: node_loc,
            relv: node_relv,
