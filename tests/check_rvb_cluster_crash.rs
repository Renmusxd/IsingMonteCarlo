...
#[test]
fn run_empty_var() {
    let mut manager = FastOps::new_from_ops(1, vec![].into_iter());
    manager.set_cutoff(3);
    let edges = EN {
    ...
    (0..100).for_each(|_| {
        manager.rvb_cluster_update(&edges, &mut state, 1, &mut rng);
        assert!(
            {
...
#[test]
fn run_empty_vars() {
    let mut manager = FastOps::new_from_ops(2, vec![].into_iter());
    manager.set_cutoff(3);
    let edges = EN {
